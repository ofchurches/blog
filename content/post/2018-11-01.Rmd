...
```{r message=FALSE, warning=FALSE}
ggraph(filter(acts_graph, centrality > 8), layout = 'graphopt', charge = .05) + 
    geom_edge_link(alpha = .8) + 
    geom_node_point(aes(colour = community, size = centrality), alpha = .8) + 
  guides(colour = FALSE) + 
    theme_graph() + 
  labs(title = str_wrap("Hairball of active legislation with more than eight edges (including loops) in South Australia November 2018", 50), subtitle = "Includng nodes with degree greater than eight (including loops)", caption = "Colour is the result of community detection")
```


This looks a bit more promising. There seem to be two majour clusters in the network. Lets have a look at the most connected acts in each of the clusters.
...
ggplot(data = acts_top, aes(x = fct_reorder(name, centrality), y = centrality, fill = community)) + 
  ...
  coord_flip() + 
  guides(fill = FALSE) + 
  labs(title = str_wrap("Top ten most connected acts in the two largest communities", 40), x = "name")
  
```


So it seems that one of the clusters (number 2) is strongly related to the management of the natural environment. The other cluster shown here (number 1) is a collection of administrative functions to do with levels of government and finance.
...
So the more recent acts are also the more interlinked acts. And from the change in density on the x axis, it seems that there are way more recent acts in the network. Also, yes, I snuck the community detection back in there for the highly connected acts and it seems that the highly connected acts we identified above as being in the largest clusters are also all quite recently accented to.
